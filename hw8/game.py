...
class Teeko2Player:
    ...
    def make_move(self, state):
        ...
        if not drop_phase:
            ...
            move = []
            (row, col) = (0, 0)
            source_x, source_y = self.nextPiece(state)
            while not state[row][col] == self.my_piece:
                (row, col) = (row, col + 1)
                if col >= 5:
                    (row, col) = (row + 1, 0)

            if state[row][col] == ' ':
                move.insert(0, [(row, col), (source_x, source_y)])
                return move
            else:
    ...
    def is_drop(self, state):
        ...
        for i in range(len(state)):
            for j in range(len(state[i])):
                # Check the num of items, if more than 8, return true:
                if not(state[i][j] ==' '):
                    count += 1
                if count >= 8:
                    return True
    ...
    def nextPiece(self, state):
        for source_x in range(len(state)):
            for source_y in range(4, len(state[source_x])):
                if state[source_x][source_y] == self.my_piece:
    ...
    def adjacent_spaces(self, color, state, current_space):
        ...
        for x in range(i_range[0], i_range[1]):
            for y in range(j_range[0], j_range[1]):
                if state[x][y] == ' ':
                    temp_result = copy.deepcopy(state)
                    temp_result[x][y] = color
                    temp_result[i][j] = ' '
    ...
    def heuristic_game_value(self, state, d, turn):
            ...
            states = self.succ(self.my_piece, state)
            
            if game_value != 0:
                return self.heuristic_game_value(state, 0, 0) if game_value == 1 else self.heuristic_game_value(state, 0, 1)
            
            elif d == 0:
                return self.eX(state, self.my_piece)

            elif turn == 1:
                return max(self.heuristic_game_value(s, d, turn) for s in states)

            elif turn == 0:
                return min(self.heuristic_game_value(s, d, turn) for s in states)
